...
| `WEBHOOK_SECRET` | Render (Web Service) — sección Environment | Clave para verificar que el webhook viene del Email Worker (incluir en la URL: `?secret=VALOR`) |
...
| `VITE_SUPABASE_URL` | Cloudflare Pages → Settings → Environment variables + `web/.env` local | URL del proyecto Supabase (formato `https://<id>.supabase.co`). Ver `web/.env.example`. |
| `VITE_SUPABASE_ANON_KEY` | Cloudflare Pages → Settings → Environment variables + `web/.env` local | Clave anon pública de Supabase (Supabase → Settings → API → Project API keys). Ver `web/.env.example`. |
...
4. Sin variables de entorno adicionales (la base URL de la API está hardcodeada como fallback en `src/api/client.js`: `https://api.myafiappdomain.com`; se puede sobreescribir con `VITE_BACKEND_URL`)
...
- **Base URL:** Hardcodeada en `ApiClient.BASE_URL` apuntando al Worker de Cloudflare en el dominio propio `https://api.myafiappdomain.com` (Iteración A3; antes `https://mytaskmyhabit-worker.fiztony.workers.dev`). Para desarrollo local se puede cambiar a `http://10.0.2.2:10000`.

### Adjuntos en tareas (Iteración A1 — Fase 1)

- **Modelo de datos:** tabla `tarea_adjuntos` (migración `server/migrations/005_create_tarea_adjuntos.sql`) con `id`, `tarea_id` (FK CASCADE a `tareas_table`), `usuario_id` (FK CASCADE a `usuarios`), `nombre_fichero`, `tipo_mime`, `tamaño_bytes`, `storage_path`, `url_publica`, `creado_en`. Índice en `tarea_id`. Al borrar una tarea, sus adjuntos se borran en cascada en la BD (el fichero físico de Storage se limpia al eliminar el adjunto vía la API; el borrado de tarea no purga Storage automáticamente en esta fase — aceptable para app personal).
- **Storage:** bucket **privado** `tarea-adjuntos`. Los ficheros NO se sirven con URL pública permanente: cada lectura genera una **URL firmada válida 1h** (`/storage/v1/object/sign`). Paths: `{usuario_id}/{tarea_id}/{timestamp}_{nombre_saneado}`.
- **Bucket autocreado:** el Worker crea el bucket (idempotente, `ensureAdjuntosBucket`) en el primer POST de adjunto usando la service-role key. No requiere paso manual en el dashboard.
- **Backend (Cloudflare Worker):** endpoints `POST/GET/DELETE /api/tareas/:id/adjuntos[/:adjuntoId]`, todos con `requireAuth` + `resolveUser` y verificación de pertenencia de la tarea al usuario. `POST` recibe `multipart/form-data` (campo `file`), valida tamaño (≤10MB) y tipo (jpg/png/gif/webp, pdf/doc/docx/xls/xlsx/txt), sube a Storage e inserta la fila (con rollback del fichero si la inserción falla). `GET` devuelve la lista con URL firmada fresca. `DELETE` borra primero de Storage y luego la fila. `handleGetTareas` anota cada tarea con `adjuntos_count` (una consulta agregada; tolerante a que la tabla aún no exista).
- **Validación en dos capas:** cliente (tamaño + extensión/MIME, feedback inmediato) y servidor (autoritativo: 413 si >10MB, 400 si tipo no permitido).
- **PWA:** `web/src/api/adjuntos.js` + `web/src/components/tareas/AdjuntosSection.jsx` (solo en modo edición de `TareaForm`, requiere tarea guardada). Lista con thumbnail para imágenes e icono Material por tipo para documentos, botón "Añadir adjunto" (selector de ficheros del navegador), barra de progreso de subida, confirmación al eliminar, y mensajes de error de tamaño/tipo. `TareaItem` muestra un badge `attach_file` cuando `adjuntos_count > 0`. `useTareas` preserva `adjuntos_count` al mergear respuestas de toggle/editar/Realtime (esas respuestas no incluyen el contador, que solo aporta el endpoint de listado).
- **Android:** `SeccionAdjuntos.kt` (composable autocontenido) integrado en `PantallaEditarTarea` (solo si `tareaId > 0`). Botón "Añadir adjunto" con menú de tres orígenes: **Cámara** (`TakePicture` + `FileProvider` + permiso `CAMERA` en runtime), **Galería** (`GetContent` `image/*`) y **Ficheros** (`OpenDocument` con MIME permitidos). Miniaturas de imagen con **Coil** (`io.coil-kt:coil-compose:2.7.0`, nueva dependencia) e icono por MIME para documentos. Progreso con `LinearProgressIndicator`, confirmación de borrado con `AlertDialog`, validación de tamaño (10MB) y tipo. `TareaCard` muestra icono `AttachFile` si `adjuntosCount > 0`. Se retiró el `contentType(application/json)` global del `defaultRequest` de Ktor (rompía las subidas multipart; ContentNegotiation ya fija el tipo al serializar objetos).
- **Fase 2 (no incluida):** adjuntos en el flujo de email (Inbound Parse) queda fuera de esta iteración.

#### Fixes posteriores (Iteración A2)

- **Android — abrir adjunto:** al pulsar un adjunto en la lista se abre. `core/network/AdjuntoOpener.kt` descarga el fichero desde la URL firmada (válida 1h) a `cacheDir/adjuntos`, lo expone con `FileProvider` (authority `${applicationId}.fileprovider`) y lanza `Intent.ACTION_VIEW` con el MIME correcto y `FLAG_GRANT_READ_URI_PERMISSION` + `FLAG_ACTIVITY_NEW_TASK`. Sirve tanto para imágenes como para documentos (el sistema elige el visor). Si no hay app capaz de abrir el tipo, se muestra un Toast. Mientras descarga, la fila del adjunto muestra un spinner.
- **PWA — adjuntar al crear:** `AdjuntosSection` pasó a `forwardRef` con dos modos. En creación (`tareaId == null`) acumula los ficheros en memoria (con preview local vía `URL.createObjectURL`) y expone `subirPendientes(nuevoId)` / `tienePendientes()`. `TareaForm` renderiza la sección también al crear; tras `onGuardar` (que ahora devuelve la tarea creada), sube los pendientes a la tarea recién creada. Si la subida falla, la tarea ya está creada y se avisa sin bloquear.
- **Android — adjuntar al crear:** `SeccionAdjuntos` admite `tareaId: Int?` y una lista `pendientes: SnapshotStateList<AdjuntoPendiente>` propiedad del padre. `PantallaCrearTarea` mantiene esa lista; al guardar, si hay pendientes usa `viewModel.crearTareaYObtenerId(...)` para obtener el id y luego `subirAdjuntoTarea(...)` por cada fichero, y refresca. El botón GUARDAR se deshabilita y muestra "GUARDANDO…" durante la subida. Sin adjuntos, se conserva el flujo `insertar` original.

#### Paso manual pendiente — aplicar la migración en Supabase

El Worker **no ejecuta migraciones** (a diferencia del antiguo backend Node/Express). Hay que aplicar la tabla una sola vez en el **SQL Editor** de Supabase (Dashboard → SQL Editor → New query), pegando el contenido de `server/migrations/005_create_tarea_adjuntos.sql` y ejecutando. El bucket `tarea-adjuntos` se crea solo en la primera subida. Hasta aplicar la migración, `adjuntos_count` sale 0 y las llamadas de adjuntos devolverán error de tabla inexistente.
...
│       │   │   └── OrdenMenu.jsx       # Dropdown ordenación/agrupación (P8)
...
│       ├── lib/
│       │   ├── supabase.js         # Cliente Supabase (null si faltan env vars — Realtime desactivado)
│       │   └── iconColors.js       # Mapeo icono Material → color (replicado de Android CategoriaUI.kt)
│       ├── hooks/
...
│       │   └── useOrden.js         # Agrupación/ordenación/dirección con localStorage (P8)
│       ├── styles/
...
| P9 | Color de iconos de categoría + campo repetición en TareaForm | Agosto 2026 | **iconColors.js:** nuevo módulo `src/lib/iconColors.js` con el mapeo completo icono→color extraído de `obtenerColorIcono()` en `CategoriaUI.kt` (Android) — 20 entradas con colores suaves, default `#757575`. **Color en TareaItem:** `getIconColor(cat.icono)` reemplaza el indigo fijo; placeholder `#e5e7eb` cuando no hay categoría asignada. **Color en Sidebar:** `SidebarItem` acepta prop `iconColor` y lo aplica via `style` cuando el ítem no está activo (activo mantiene el indigo del CSS). **Color en CategoriaItem:** reemplaza el badge de texto monospace por un `<span class="material-icons c-item-icon-mat">` de 36×36px con fondo sutil `rgba(0,0,0,0.05)` y color del mapeo. **Preview en TareaForm:** icono Material de la categoría seleccionada visible junto al `<select>` (`.m-cat-wrap` flex + `.m-cat-icon-preview`). **Repetición en TareaForm:** selector con las 5 opciones exactas de la app Android ("Sin repetición" / "Una vez al día" / "Una vez a la semana" / "Una vez al mes" / "Una vez al año"); campos `repeticion_fin` (date) y `repeticion_veces` (number) aparecen condicionalmente cuando hay repetición activa; ambos opcionales; `formToBody` incluye los tres campos; `tareaToForm` los precarga al editar. `repeticion_contador` no se envía (el backend lo gestiona internamente). Build: 168 módulos, 490 kB. Commit `b68d0d1` pusheado a master. |
| P8 | Icono categoría a la izquierda + menú de ordenación/agrupación | Agosto 2026 | **Icono categoría izquierda:** `TareaItem` mueve el icono Material entre el checkbox y el título; siempre visible — si no hay categoría muestra `circle` en `#e5e7eb` para mantener la alineación. El icono de repetición (`repeat`) permanece en `ti-right`. **useOrden:** nuevo hook con tres ejes de estado (`agrupacion`/`ordenarPor`/`orden`) persistidos en `localStorage` (clave `mtmh_orden`); memoiza `sortFn` para aplicarla a `tareasFiltradas` y a cada grupo del panel. **OrdenMenu:** dropdown posicionado absoluto bajo el header (backdrop transparente fijo para cerrar al clic fuera); tres secciones — "Agrupar por" (Tiempo/Prioridad/Categoría, oculta en bandeja y completadas), "Ordenar por" (Fecha límite/Prioridad/Creación/Título), "Orden" (Asc/Desc). **PanelCabecera:** botón `swap_vert` que activa/cierra el dropdown y se ilumina cuando está abierto. **Tareas.jsx:** `effectiveDisplayMode` sobreescribe la agrupación del sidebar con la del menú — añade `sections-prio` (ALTA/MEDIA/BAJA) y `sections-cat-group` (secciones dinámicas por categoría con orden de la lista); cuando `agrupacion=tiempo` con filtro de todas+cat, convierte el `flat` original en `sections-all`. La `sortFn` se aplica en `tareasFiltradas` (lista completa) y los grupos heredan ese orden. Build: 167 módulos, 488 kB. Commit `c8d7421` pusheado a master. |
| P10 | Selector de hora: reemplazar drum por input+popover (PC-first) | Septiembre 2026 | `TimePicker.jsx` reescrito como campo de texto + popover inline (sin modal). Escritura directa con formato `HH:mm`: auto-inserción de `:` al 2º dígito, soporte de entrada `HHMM` sin dos puntos, validación y commit en Tab/Enter/clic fuera, reset en Escape. Popover con: atajos rápidos (09:00/12:00/14:00/18:00/20:00), grid de horas 6×4, fila de minutos cada 15′ (:00/:15/:30/:45), botón "Quitar hora". Cierre al pulsar fuera o Escape. `TareaForm.jsx`: eliminado estado `showTimePicker` y botón trigger; reemplazado por `<TimePicker id="tf-hora" value=... onChange=... disabled=... />`. CSS `tareas.css`: eliminados `.m-time-btn` y bloque drum (`tp-overlay`, `tp-panel`, `tp-drum*`, `tp-highlight`, `tp-mask*`, `tp-footer`, `tp-btn-*`); añadido bloque popover (`.tp-wrap`, `.tp-row`, `.tp-pop`, `.tp-atajos`, `.tp-hours`, `.tp-mins`, `.tp-clear`). Build: `npm run build` OK. Commit pusheado a master. |
| BUGFIX | Voz: recorte de frases al crear tarea por voz | Agosto 2026 | **Bug:** frases de 3+ palabras dictadas por voz se guardaban recortadas a solo la ultima palabra (ej. 'Alerta de habitos criticos' -> 'Criticos'). **Causa raiz:** en VozTaskParser.kt, la regex de extraerFechaDiaMes() (reutilizada en limpiarTitulo()) para detectar fechas tipo '15 de mayo' aceptaba cualquier palabra como mes, coincidiendo con patrones genericos 'palabra de palabra' como 'alerta de habitos', y limpiarTitulo() eliminaba ese fragmento del titulo completo. **Fix:** se restringio el grupo de mes a una lista cerrada de meses reales (MESES_REGEX, enero-diciembre). Verificado con pruebas de regex aisladas (ya no matchea 'alerta de habitos'/'tarea de casa', sigue reconociendo 'el 15 de mayo'). gradlew assembleDebug BUILD SUCCESSFUL, gradlew installDebug instalado en dispositivo sin desinstalar (datos preservados). |
| A3 | Backend a dominio propio (`api.myafiappdomain.com`) | Septiembre 2026 | El Worker principal pasa a servirse en el dominio propio **`https://api.myafiappdomain.com`** (antes `mytaskmyhabit-worker.fiztony.workers.dev`). Actualizada la URL en: `ApiClient.BASE_URL` (Android), fallback de `client.js` (PWA) y `VITE_BACKEND_URL` de `web/.env.example`, y `BACKEND_URL` por defecto del `email-worker/src/index.js` (`.../webhooks/email`). Verificado `GET /health` → 200. CORS del Worker sin cambios (la PWA sigue en `mytaskmyhabitpwa.pages.dev`, que permanece en la whitelist). Android assembleDebug BUILD SUCCESSFUL + installDebug; PWA `npm run build` OK; email-worker redeploy con `wrangler deploy`. Docs (`task-worker/README.md`, `docs/README.md`) actualizadas. |
| A2 | Adjuntos: abrir en Android + adjuntar al crear (Android y PWA) | Septiembre 2026 | **Android abrir adjunto:** `AdjuntoOpener.kt` descarga la URL firmada a caché, la expone con `FileProvider` y lanza `ACTION_VIEW` con el MIME (imágenes y documentos); spinner por fila y Toast si no hay visor. **PWA adjuntar al crear:** `AdjuntosSection` → `forwardRef` con modo creación (ficheros pendientes en memoria + preview local); `TareaForm` los sube tras crear la tarea (`handleGuardar` devuelve la tarea con su id). **Android adjuntar al crear:** `SeccionAdjuntos` con `tareaId: Int?` + lista `pendientes` propiedad del padre; `PantallaCrearTarea` crea la tarea (`crearTareaYObtenerId`), sube cada pendiente y refresca; botón "GUARDANDO…". Build PWA OK y assembleDebug BUILD SUCCESSFUL; installDebug en dispositivo (sin uninstall). |
| A1 | Adjuntos en tareas (Fase 1): Android + PWA | Septiembre 2026 | **BD:** migración `005_create_tarea_adjuntos.sql` (tabla `tarea_adjuntos`, FK CASCADE, índice). **Storage:** bucket privado `tarea-adjuntos` autocreado por el Worker (URLs firmadas 1h, sin público permanente). **Worker:** endpoints GET/POST/DELETE `/api/tareas/:id/adjuntos[/:adjuntoId]` con validación tamaño (10MB) + tipo (jpg/png/gif/webp, pdf/doc/docx/xls/xlsx/txt), subida a Storage con rollback, y `adjuntos_count` en el listado de tareas. Desplegado a producción. **PWA:** `api/adjuntos.js` + `AdjuntosSection.jsx` en `TareaForm` (edición): lista con thumbnail/icono por tipo, subir con progreso, eliminar con confirmación, errores de tamaño/tipo; badge `attach_file` en `TareaItem`. Build OK (171 módulos). **Android:** `SeccionAdjuntos.kt` en `PantallaEditarTarea` con cámara/galería/ficheros (permiso CAMERA + FileProvider), thumbnails con Coil, progreso, confirmación y validaciones; icono `AttachFile` en `TareaCard`; se retiró el Content-Type JSON global de Ktor para permitir multipart. assembleDebug BUILD SUCCESSFUL + installDebug en dispositivo (sin uninstall). **Paso manual pendiente:** aplicar `005_...sql` en el SQL Editor de Supabase (el Worker no corre migraciones). Fase 2 (adjuntos por email) fuera de alcance. |
